...
long signed front_right_counter = 0;

//attach interrupts to pin (rising edge)
void encoders_init()
{
 ext_irq_register(encoder1_A,interrupt_front_left);
 ext_irq_register(encoder2_A, interrupt_front_right);
 ext_irq_register(Encoder3_A, interrupt_back_left);
 ext_irq_register(Encoder4_A, interrupt_back_right);
}
